...
#			+ xlim(limits = range(df$temp_var, na.rm = TRUE))
	...
	if (show_percent_labels){
		p1 <- p0 +   geom_text(aes( label = scales::percent(after_stat(prop), accuracy = 0.1)
			, y = after_stat(prop)
	...
	if (shift_axis){
		p1 <- p0 + coord_flip()
		if (show_percent_labels){
			p1 <- p1 +   geom_text(aes(label = scales::percent(after_stat(prop), accuracy = 0.1)
				, y = after_stat(prop)
...
multiresFunc <- function(df, var_patterns, question_labels
	...
	if(sum_over_N){
		p1 <- (p1
 			+ geom_bar(aes(y = (after_stat(count))/N))
 			+ scale_y_continuous(labels = percent)
		)
		if (show_percent_labels){
			if (!shift_axis){
				p2 <- p1 +   geom_text(aes( label = scales::percent(after_stat(count)/N)
					, y = after_stat(count)/N
				), stat= "count", vjust = -.1, hjust = 0)
			} else {
				p2 <- p1 +   geom_text(aes( label = scales::percent(after_stat(count)/N)
					, y = after_stat(count)/N
		...
		if (show_percent_labels){
			if (!shift_axis){
				p2 <- p1 +   geom_text(aes( label = scales::percent(after_stat(count)/sum(after_stat(count)))
